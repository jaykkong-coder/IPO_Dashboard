...
## 표지 (SVG 벡터)
- 흰 배경 + 레드 리본 웨이브(**단일 붓질 경로**) + 하단 회색 바
- **리본은 path 하나로 그린다.** 밴드 2개를 겹쳐 접힘(fold)을 만들면 두 밴드가 만나는 중앙에서
  선이 끊긴 것처럼 보인다(2026-08-04 Jaykong 지적으로 템플릿 교체). 같은 경로를 세 번 깔아
  입체감을 낸다 — ① 그림자(`#40403f` 0.14 + blur 42, translate(28,82)) ② 본체(`rg1` 그라데이션)
  ③ 상단 하이라이트(흰색 0.14, 아래 에지만 위로 당긴 경로)
